        ...
        stage('Build & Push') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub', passwordVariable: 'dockerpass', usernameVariable: 'dockeruser')]) {
                    sh 'cd project-webshop/app/recommendation && docker build -t recommendation .'
                    sh 'docker login -u $dockeruser -p $dockerpass'
                    sh 'docker tag recommendation $dockeruser/recommendation:latest'
                    sh 'docker push $dockeruser/recommendation:latest'
                    // Remove all images 
