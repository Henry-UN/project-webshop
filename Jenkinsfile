        ...
        stage('Build & Push') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub', passwordVariable: 'dockerpass', usernameVariable: 'dockeruser')]) {
                    sh 'cd project-webshop/app/productcatalogue && docker build -t productcatalogue .'
                    sh 'docker login -u $dockeruser -p $dockerpass'
                    sh 'docker tag productcatalogue $dockeruser/productcatalogue:latest'
                    sh 'docker push $dockeruser/productcatalogue:latest'
                    // Remove all images 
